...
#pragma mark - get
    ...
    ofMatrix4x4 ARAnchorManager::getAnchorMatrix(int index)
    {
        return anchors[index].modelMatrix;
    }
    
    ofMatrix4x4 ARAnchorManager::getLastAnchorMatrix()
    {
        ofMatrix4x4 mat;
        if(!anchors.empty())
        {
            mat = getAnchorMatrix(anchors.size() - 1);
        }
        else
        {
            ofLogError("Matrix is null! check the anchors array");
        }
        
        return mat;
        
//            cout << "anchors num: " << $Context(AR)->processor->getNumAnchors() << endl;
//            ARAnchor * anchor = session.currentFrame.anchors[0];
        
//        if(session.currentFrame.anchors[session.currentFrame.anchors.count - 1] == nullptr)
//        {
//            cout << "nullptr!!" << endl;
//        }
        //        getAnchorMatrix(session.currentFrame.anchors.count - 1);
        //        matrix_float4x4 transform = session.currentFrame.anchors.lastObject.transform;
